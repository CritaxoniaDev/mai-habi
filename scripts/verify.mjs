...
check(
  ...
  () => !fs.isSafePath("C:\\Windows\\system32"),
);
check("accepts ordinary paths", () =>
  fs.isSafePath("src/components/Button.tsx"),
);
...
check(
  "keeps source files on import",
  () => !fs.shouldIgnoreImportPath("src/App.tsx"),
);
check(
  "strips a single common root",
...
check("renames a folder and its descendants", () => {
  ...
  return (
    Boolean(next["source/App.tsx"] && next["source/lib/util.ts"]) &&
    !next["src/App.tsx"]
  );
});
...
check(
  ...
  () => fs.detectEntryFile(seed) === "src/main.tsx",
);
check(
  "detects a root HTML entry",
  () =>
    fs.detectEntryFile(
      shared.filesFromRecord({ "index.html": "<h1>Hello</h1>" }),
    ) === "index.html",
);
check(
  "honours a configured entry",
  () => fs.detectEntryFile(seed, "src/App.tsx") === "src/App.tsx",
);
...
check(
  "knows tsx is typescript for monaco",
  () => fs.languageForPath("src/App.tsx") === "typescript",
);
check(
  "knows jsx is javascript for monaco",
  () => fs.languageForPath("src/App.jsx") === "javascript",
);
check(
  "detects additional source languages from new filenames",
  () =>
    fs.languageForPath("script.py") === "python" &&
    fs.languageForPath("main.rs") === "rust" &&
    fs.languageForPath("schema.graphql") === "graphql",
);
check("describes the detected language for the new-file UI", () => {
  const detected = fs.detectLanguageForPath("main.go");
  return detected.recognized && detected.id === "go" && detected.label === "Go";
});
check(
  "maps schema languages",
  () => fs.fileLanguage("a.tsx") === "typescriptreact",
);
...
async function compileFixture(files, entry, options = {}) {
  try {
    const result = await nodeBuild(
      bundler.createBuildOptions({ files, entry, minify: false, ...options }),
    );
    ...
    return {
      ok: false,
      errors: (error.errors ?? []).map((entry) => entry.text),
    };
  }
}
...
check("aggregates imported CSS into its own output", async () => {
  ...
  return (
    result.ok &&
    result.css.includes("margin: 0") &&
    !result.js.includes("margin: 0")
  );
});
...
check("runs inline HTML scripts after adopting the document", async () => {
  ...
  return (
    result.ok &&
    result.js.includes("dataset.ready") &&
    result.js.includes("append")
  );
});
...
check("reports a missing script referenced by HTML", async () => {
  ...
  return (
    !result.ok &&
    result.errors.some((message) => message.includes("./missing.js"))
  );
});
  ...
  return (
    ...
    result.errors.some(
      (message) =>
        ...
        message.includes("not available in this playground"),
    )
  );
});
...
check("names the package root for a deep unknown import", async () => {
  ...
  return (
    !result.ok &&
    result.errors.some((message) => message.includes('"@scope/pkg"'))
  );
});

check(
  "reports an unresolved relative import with both files named",
  async () => {
    const result = await compileFixture(
      { "a.tsx": `import './missing';` },
      "a.tsx",
    );
    return (
      !result.ok &&
      result.errors.some(
        (message) => message.includes("./missing") && message.includes("a.tsx"),
      )
    );
  },
);
...
check("reports a missing entry file", async () => {
  const result = await compileFixture(
    { "a.tsx": "export default 1;" },
    "src/nope.tsx",
  );
  return (
    !result.ok &&
    result.errors.some((message) => message.includes("was not found"))
  );
});
...
check("compiles a project using the curated libraries", async () => {
  ...
  if (!result.ok) throw new Error(result.errors.join("; "));
  return (
    result.js.includes("AnimatePresence") && result.js.includes("ReactLenis")
  );
});
...
check(
  "a missing html image is left alone rather than failing the build",
  async () => {
    const result = await compileFixture(
      {
        "index.html":
          '<!doctype html><html><body><img src="./gone.png"></body></html>',
      },
      "index.html",
    );
...
check("a large asset compiles but warns", async () => {
  ...
  return (result.warnings ?? []).some((warning) =>
    warning.text.includes("inlined into the build"),
  );
});

check("a project snapshot keeps images binary on the way back", () => {
  const files = shared.filesFromRecord({
    "src/App.tsx": "export default null;",
    "logo.png": PNG_BASE64,
  });
  return (
    files["logo.png"].encoding === "base64" &&
    files["src/App.tsx"].encoding === "utf8"
  );
});
...
check("every allowed package is staged into /runtime", () => {
  for (const [name, url] of Object.entries(runtime.ALLOWED_PACKAGES)) {
    ...
    if (!existsSync(file))
      throw new Error(`${name} points at a missing file: ${url}`);
  }
  return true;
});

check(
  "framer-motion and motion/react are the same module",
  () =>
    runtime.ALLOWED_PACKAGES["framer-motion"] ===
    runtime.ALLOWED_PACKAGES["motion/react"],
);
...
check(
  "the preview sandbox withholds every permission but scripts",
...
check("closing tags inside compiled output cannot break out", () => {
  ...
  return (
    body.includes("<\\/script>") && !body.includes('console.log("</script>')
  );
});
...
check("the import map points react at the platform runtime", () => {
  const map = JSON.parse(
    previewDocument.slice(
      previewDocument.indexOf('<script type="importmap">') +
        '<script type="importmap">'.length,
      previewDocument.indexOf(
        "</script>",
        previewDocument.indexOf('<script type="importmap">'),
      ),
    ),
  ...
  return (
    map.imports.react ===
      "https://play.example.com/runtime/react.production.js" &&
    map.imports["react-dom/client"].endsWith(
      "/runtime/react-dom-client.production.js",
    )
  );
});
...
check("tailwind is absent unless the project enables it", () => {
  ...
  return (
    !previewDocument.includes("tailwind") &&
    withTailwind.includes("tailwind-browser.js")
  );
});

check(
  "the preview bridge is valid javascript and forwards console output",
  () => {
    const start = previewDocument.indexOf("<script>") + "<script>".length;
    const body = previewDocument.slice(
      start,
      previewDocument.indexOf("</script>", start),
    );
    new vm.Script(body);
...
check("share ids are unique and url safe", () => {
  ...
  return (
    ids.size === 500 && [...ids].every((id) => /^[0-9a-zA-Z]{12}$/.test(id))
  );
});

check(
  "editor and viewer routes share one origin",
  () =>
...
check(
  "cloud stays disabled without configuration",
  () => shared.isCloudEnabled() === false,
);
...
check("every template compiles", async () => {
  for (const template of shared.TEMPLATES) {
    ...
    if (!entry) {
      ...
      if (!generated)
        throw new Error(`${template.id}: no entry and nothing to synthesise`);
    ...
    if (!result.ok)
      throw new Error(`${template.id}: ${result.errors.join("; ")}`);
  }
  return true;
});
...
check("the motion template demonstrates the provided libraries", async () => {
  ...
  for (const specifier of ["framer-motion", "lenis/react", "clsx"]) {
    if (!source.includes(specifier))
      throw new Error(`the sample never imports ${specifier}`);
  }

  // And it has to actually build, not merely mention them.
  const result = await compileFixture(
    template.files,
    template.settings.entryFile,
  );
  if (!result.ok) throw new Error(result.errors.join("; "));
...
check("starter samples show a local component import", () => {
  for (const id of ["react-ts", "react-js"]) {
    ...
    const app =
      template.files[id === "react-ts" ? "src/App.tsx" : "src/App.jsx"];
    const component =
      id === "react-ts"
        ? "src/components/Counter.tsx"
        : "src/components/Counter.jsx";

    if (!app.includes("./components/Counter"))
      throw new Error(`${id} does not import a component`);
    if (!template.files[component])
      throw new Error(`${id} is missing ${component}`);
  }
  return true;
});
...
check("the viewer compiles a no-entry project from its snapshot", async () => {
  ...
  let entry = fs.detectEntryFile(
    shared.filesFromRecord(sources),
    template.settings.entryFile,
  );
  ...
  if (!entry) {
    ...
    if (!generated)
      throw new Error("no mount could be synthesised for the snapshot");
    sources[generated.path] = generated.contents;
    entry = generated.path;
  }
...
export default function Page() {
  ...
  return (
    <main className={inter.className}>
      ...
      <Script id="preview-script" strategy="afterInteractive">
        {'window.__previewScriptRan = true;'}
      </Script>
...
`,
  ...
  if (!generated)
    throw new Error("the Next page did not yield a mountable root");
  const result = await compileFixture(
    { ...files, [generated.path]: generated.contents },
    generated.path,
  );
  if (!result.ok) throw new Error(result.errors.join("; "));
  return true;
...
check(
  "server-only next modules are rejected with a clear message",
  async () => {
    const files = {
...
`,
    ...
    if (!result.errors.some((text) => /server/i.test(text))) {
      throw new Error(
        `error did not explain the server-only limitation: ${result.errors.join("; ")}`,
      );
    }
    return true;
  },
...
check("only the tailwind template enables tailwind", () => {
  const enabled = shared.TEMPLATES.filter(
    (template) => template.settings.tailwind,
  );
...
check("created projects materialise directories", () => {
  ...
  return (
    files.src.type === "directory" &&
    project.settings.entryFile === "src/main.tsx"
  );
});
...
check(
  "a wrapping folder is dropped when an import becomes the project",
  async () => {
    const result = await fs.importFromZip(archive());
    return Boolean(
      result.files["index.html"] && result.files["assets/logo.svg"],
    );
  },
);
...
check("structure is kept when merging into an existing folder", async () => {
  ...
  return Boolean(
    result.files["widget/index.html"] && result.files["widget/assets/logo.svg"],
  );
});
...
check("an imported binary keeps its base64 encoding", async () => {
  const zip = zipSync({
    "pack/logo.png": new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]),
  });
...
check(
  "tsx and jsx share the React accent",
  () =>
    toneOf("src/App.tsx") === "text-lang-react" &&
    toneOf("src/App.jsx") === "text-lang-react",
);

check(
  "plain typescript is distinct from a component",
  () =>
...
check(
  "images are recognised, including svg",
  () =>
    toneOf("logo.png") === "text-lang-image" &&
    toneOf("logo.svg") === "text-lang-image",
);

check(
  "known filenames beat their extension",
  () =>
...
check(
  "dotfiles and env files read as configuration",
  () =>
...
check(
  "an unknown extension falls back to a neutral file",
...
check("languages use a real logo, other categories use a glyph", () => {
  ...
  for (const path of withLogo) {
    if (!icons.fileKind(path).logo)
      throw new Error(`${path} has no language logo`);
  }
  for (const path of withGlyph) {
    if (icons.fileKind(path).logo)
      throw new Error(`${path} should not claim a language logo`);
  }

  return true;
});

check("language and framework logos come from the cached SVGL API", () => {
  const route = readFileSync("apps/web/src/app/api/svgl/route.ts", "utf8");
  const logo = readFileSync("apps/web/src/components/SvglLogo.tsx", "utf8");

  return (
    route.includes("https://api.svgl.app") &&
    route.includes('category("framework")') &&
    route.includes('category("language")') &&
    route.includes("stale-while-revalidate") &&
    logo.includes("/api/svgl")
  );
});

check(
  "classification ignores the folder path",
